...
function _mt:is_0_14()
  ...
  local rows, err = self.connection:query "SELECT to_regclass('schema_migrations') AS \"name\";"
  if not rows then
    return nil, err
  end

  if rows[1] and rows[1].name == "schema_migrations" then
    rows, err = self.connection:query 'SELECT "id", "migrations" FROM "schema_migrations";'
  else
    return nil, nil, true
  end

  if not rows then
    return nil, err, nil, "schema_migrations"
  end
  ...
  for name, migrations in pairs(needed_migrations) do
    ...
    for _, needed_migration in ipairs(migrations) do
      ...
      if not found then
        return nil, nil, nil, name, needed_migration
      end
    end
  end
