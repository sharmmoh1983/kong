...
do
  ...
  function CassandraConnector:is_0_14()
    ...
    rows, err = self.connection:execute(cql, { self.keyspace, "schema_migrations" })
    if not rows then
      return nil, err
    end

    if rows[1] and rows[1].count > 0 then
      rows, err = self.connection:execute("SELECT id, migrations FROM schema_migrations")
    else
      return nil, nil, true
    end
